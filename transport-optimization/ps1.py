...
def brute_force_cow_transport(cows,limit=10):
    ...
    # dla danego schematu transportu
    for transport in get_partitions(cows):
        transApproved = True
        # dla każdego ładunku w schemacie 
        for i in range(len(transport)):
            totalWeight = 0
            #dla każdego elementu danego ladunku
            for j in range(len(transport[i])):
                #zrob sumę ładunku
                totalWeight += cows.get(transport[i][j])
            #jeżeli waga ładunku jest powyżej limitu
            if totalWeight > limit:
                #odrzucamy cały schemat transportu
                transApproved = False
                break
        if transApproved: 
            result.append(transport)
    
    #wybrac sposrod listy schematow transportow ten ktory jest najkrotszy
    #zawiera min ladunkow
    min = len(cows)+1
    finalResult = []
